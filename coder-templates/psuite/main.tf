...
resource "docker_volume" "psuite" {
  name = "coder-${data.coder_workspace.me.id}-psuite"
  driver = "local"

  # Don't forget to add docker cidr in nfs allowlist
  driver_opts = {
    type   = "nfs"
    o      = "addr=truenas.cosmos.cboxlab.com,rw,nfsvers=4" # Replace with your NFS server IP and options
    device = ":/mnt/main/k8s/psuite"      # Replace with the NFS directory path
  }
...
resource "docker_container" "workspace" {
  ...
  volumes {
    container_path = "/psuite"
    volume_name    = docker_volume.psuite.name
    read_only      = false
  }
